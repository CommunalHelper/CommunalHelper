...
public class CustomSummitGemManager : Entity
{
        ...
        : base(data.Position + offset)
    {
        ...
        // big ol' one liner
        heartOffset = Array.ConvertAll(data.Attr("heartOffset").Split(','), float.Parse).ToVector2();
        /*
    ...
    private class Gem : Entity
    {
            ...
            : base(position)
        {
            ...
            ParticleColor = Everest.Content.TryGet(GFX.Game.RelativeDataPath + "collectables/summitgems/" + id + "/gem.meta", out ModAsset asset) &&
                asset.TryDeserialize(out CustomSummitGem.ColorMeta meta)
                ? Calc.HexToColor(meta.Color)
                : CustomSummitGem.GemColors[Index];
        }
