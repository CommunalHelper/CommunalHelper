...
namespace Celeste.Mod.CommunalHelper.Entities {
    class MultiDashHeartGem : HeartGem {
        ...
        class DemoCutscene : DialogCutscene {
            public DemoCutscene(EventTrigger trigger, Player player, string eventID)
                : base(eventID, player, false) { }
        }

        private static MethodInfo m_HeartGem_Collect = typeof(HeartGem).GetMethod("Collect", BindingFlags.NonPublic | BindingFlags.Instance);

        private int health = 3;
        private string[] cutscenes = new string[3];

        private DynData<HeartGem> baseData;
