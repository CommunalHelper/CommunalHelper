...
public class PlayerSeekerBarrierRenderer : Entity
{
    ...
    private class Edge
    {
        ...
        {
            return view.Left < Parent.X + Max.X &&
    ...
    private readonly List<PlayerSeekerBarrier> list = new();
    ...
    {
        return a == b || (a & b & ~Tiling.Occupied) != Tiling.Empty;
    }
            ...
            for (int i = (int) barrier.X / 8; i < barrier.Right / 8f; i++)
            {
                for (int j = (int) barrier.Y / 8; j < barrier.Bottom / 8f; j++)
                {
                    for (int k = 0; k < edgeBuildOffsets.Length; k++)
                    {
                        ...
                        Point perp = new(-offset.Y, offset.X);
                        ...
                        {
                            Point at = new(i, j);
                            Point to = new(i + perp.X, j + perp.Y);
