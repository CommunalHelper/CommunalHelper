...
public class NoOverlayLookout : Entity
{
    ...
    private IEnumerator LookRoutine(Player player)
    {
        ...
        while (!Input.MenuCancel.Pressed && !Input.MenuConfirm.Pressed && !Input.Dash.Pressed && !Input.Jump.Pressed && interacting)
        {
            ...
            if (sprite.CurrentAnimationID is not "lookLeft" and not "lookRight")
            {
                if (value.X == 0f)
                {
                    if (value.Y == 0f)
            ...
            else
            {
                ...
                if (nodePercent < 0.25f && node > 0)
                {
                    ...
                    SimpleCurve simpleCurve = new(begin, end, from);
                ...
                else if (nodePercent > 0.75f && node < nodes.Count - 1)
                {
                    ...
                    SimpleCurve simpleCurve = new(begin, end, to);
        ...
        if (atSummitTop)
        {
            ...
            Coroutine component = new(level.ZoomTo(new Vector2(160f, 90f), 2f, duration));
            Add(component);
